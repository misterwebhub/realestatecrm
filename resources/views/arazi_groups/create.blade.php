...
<div class="container-fluid py-3">
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <form action="{{ route('arazi-groups.store') }}" method="POST">
                @csrf
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <select name="map_name" id="map_name" class="form-select @error('map_name') is-invalid @enderror" required>
                            ...
                            @foreach($mapNames as $name)
                                <option value="{{ $name }}" @selected(old('map_name') === $name)>{{ $name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-12">
                        ...
                        <select name="arazi_ids[]" id="arazi_ids" class="form-select @error('arazi_ids') is-invalid @enderror"
                            ...
                            @php $oldIds = collect(old('arazi_ids', [])); @endphp
                            @foreach($arazis as $arazi)
                                <option value="{{ $arazi->id }}" @selected($oldIds->contains($arazi->id))>
